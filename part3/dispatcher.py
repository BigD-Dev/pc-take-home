...
class TokenBudgetDispatcher:
    def __init__(self, token_budget: int, max_concurrent: int = 10):
        ...
        self._lock           = asyncio.Lock()  # asyncio.Lock for atomic check+update
    ...
    async def dispatch(self, fn: Callable[[], Awaitable[dict]]) -> dict:
        ...
        async with self.semaphore:
            try:
                result = await fn()  # fn() runs outside the lock so LLM calls stay concurrent
            except Exception:
            ...
            async with self._lock:  # lock held only for check/update and no await inside so no context switch possible
    ...
    async def dispatch_with_backoff(
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                raise  # budget exceeded or circuit open, retrying wont help
            except Exception:
                if attempt == max_retries:
                    raise
                # full jitter - window grows with each attempt, not flat
